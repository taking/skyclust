...
func NewService(db database.Service, eventBus events.Bus) Service {
	return &service{
		db:       db,
		eventBus: eventBus,
	}
...
func (s *service) Plan(ctx context.Context, workspaceID, config string) (*Execution, error) {
	...
	// Publish event
	s.eventBus.PublishToWorkspace(ctx, workspaceID, &events.Event{
...
func (s *service) simulateExecution(ctx context.Context, execution *Execution, config string) {
	...
	// Save to database
	s.db.UpdateExecution(ctx, execution)

	// Publish completion event
	s.eventBus.PublishToWorkspace(ctx, execution.WorkspaceID, &events.Event{
