...
// createAzureResourceManagerClient: Azure Resource Manager 클라이언트를 생성합니다
func (s *Service) createAzureResourceManagerClient(ctx context.Context, creds *AzureCredentials) (*armresources.ClientFactory, error) {
	...
	clientFactory, err := armresources.NewClientFactory(creds.SubscriptionID, cred, nil)
	if err != nil {
		return nil, domain.NewDomainError(domain.ErrCodeProviderError, fmt.Sprintf("failed to create Azure Resource Manager client: %v", err), 502)
	}
