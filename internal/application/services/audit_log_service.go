...
func (s *auditLogService) ExportAuditLogs(filters domain.AuditLogFilters, format string) ([]byte, error) {
	...
	// Set reasonable limit for export (max 10000 records)
...
func (s *auditLogService) exportAuditLogsToCSV(logs []*domain.AuditLog) ([]byte, error) {
	var buf strings.Builder
	writer := csv.NewWriter(&buf)
...
func (s *auditLogService) exportAuditLogsToJSON(logs []*domain.AuditLog) ([]byte, error) {
