...
func (h *Handler) GetAuthURL(c *gin.Context) {
	...
	if provider == "" {
		responses.BadRequest(c, "Provider is required")
		return
	}
	...
	if _, err := rand.Read(stateBytes); err != nil {
		responses.InternalServerError(c, "Failed to generate state parameter")
		return
	}
	...
	if err != nil {
		responses.InternalServerError(c, "Failed to get auth URL")
		return
	}
...
func (h *Handler) Callback(c *gin.Context) {
	...
	if provider == "" || code == "" {
		responses.BadRequest(c, "Provider and code are required")
		return
	}
	...
	if err != nil {
		responses.InternalServerError(c, "Failed to process callback")
		return
	}
...
func (h *Handler) Login(c *gin.Context) {
	...
	if err != nil {
		responses.InternalServerError(c, "Failed to login")
		return
	}
...
func (h *Handler) Logout(c *gin.Context) {
	...
	responses.OK(c, gin.H{"message": "Logout successful"}, "Logout successful")
}
...
func (h *Handler) GetLogoutURL(c *gin.Context) {
	...
	if err != nil {
		responses.InternalServerError(c, "Failed to get logout URL")
		return
	}
