...
func (p *GCPProvider) Initialize(config map[string]interface{}) error {
	...
	fmt.Printf("GCP provider initialized for project: %s, region: %s\n", p.projectID, p.region)
	return nil
}
		...
		for {
			...
			if len(instance.NetworkInterfaces) > 0 {
				...
				if ni.AccessConfigs != nil && len(ni.AccessConfigs) > 0 {
					if ni.AccessConfigs[0].NatIP != nil {
...
// getZonesInRegion returns zones in the specified region
func (p *GCPProvider) getZonesInRegion(ctx context.Context, region string) ([]string, error) {
...
func (p *GCPProvider) CreateInstance(ctx context.Context, req interfaces.CreateInstanceRequest) (*interfaces.Instance, error) {
	...
	instanceConfig := &computepb.Instance{
		Name:        &req.Name,
		MachineType: &[]string{fmt.Sprintf("zones/%s/machineTypes/%s", zone, req.Type)}[0],
		Disks: []*computepb.AttachedDisk{
			{
				InitializeParams: &computepb.AttachedDiskInitializeParams{
					DiskSizeGb:  &[]int64{20}[0], // Default disk size
...
func (p *GCPProvider) DeleteInstance(ctx context.Context, instanceID string) error {
	// Find the instance in all zones
	zones, err := p.getZonesInRegion(ctx, p.region)
	if err != nil {
	...
	for _, zone := range zones {
		...
		if err == nil {
			fmt.Printf("Deleting GCP instance: %s\n", instanceID)
			return nil
		}
	}
...
func (p *GCPProvider) GetInstanceStatus(ctx context.Context, instanceID string) (string, error) {
	// Find the instance in all zones
	zones, err := p.getZonesInRegion(ctx, p.region)
	if err != nil {
		return "", fmt.Errorf("failed to get zones: %w", err)
...
func (p *GCPProvider) ListRegions(ctx context.Context) ([]interfaces.Region, error) {
	...
	for {
		...
		if len(parts) >= 2 {
			...
			regionMap[region] = zone.GetDescription()
		}
	}
