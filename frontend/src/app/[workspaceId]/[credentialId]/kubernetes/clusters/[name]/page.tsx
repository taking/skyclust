 ...
 * 클러스터 상세 페이지
 * 
 * 새로운 라우팅 구조: /{workspaceId}/{credentialId}/kubernetes/clusters/{name}
 */
...
function KubernetesClusterDetailPageContent() {
  ...
  useEffect(() => {
    if (!isLoadingCluster && !cluster && clusterName && workspaceId && credentialId && selectedProvider) {
  ...
  }, [isLoadingCluster, cluster, clusterName, workspaceId, credentialId, selectedProvider, router, region]);
  ...
  if (!workspaceId || !credentialId || !selectedProvider) {
    return (
      <Layout>
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">
              {!workspaceId ? 'No Workspace Selected' : !credentialId ? 'No Credential Selected' : 'Select a Provider'}
            </h2>
            <Button onClick={() => {
              if (!workspaceId) {
                router.push('/workspaces');
              } else if (!credentialId) {
                router.push(buildManagementPath(workspaceId, 'credentials'));
              } else {
                const path = buildResourcePath(workspaceId, credentialId, 'kubernetes', '/clusters');
                router.push(path);
              }
            }}>
              {!workspaceId ? 'Manage Workspaces' : !credentialId ? 'Manage Credentials' : 'Back to Clusters'}
            </Button>
          </div>
        </div>
      </Layout>
        ...
        {selectedCredentialId && cluster && (
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as typeof activeTab)} className="space-y-4">
            ...
            <TabsContent value="overview" className="space-y-4">
              <ClusterDetailOverviewTab cluster={cluster} />
            </TabsContent>
