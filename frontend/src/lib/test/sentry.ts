...
export async function testSentryMessage(): Promise<void> {
  if (typeof window === 'undefined') {
    log.error('This function can only be called in the browser', new Error('Browser only function'));
    return;
  }
  ...
  log.info('[Sentry Test] Test message sent! Check your Sentry dashboard.');
}
...
export async function testSentryError(): Promise<void> {
  ...
  log.info('[Sentry Test] Test error sent! Check your Sentry dashboard.');
}
...
if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
  ...
  log.info('[Sentry Test] Test functions available:', {
