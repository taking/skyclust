...
class OfflineQueueManager {
  ...
  private loadQueue(): void {
    ...
    try {
      ...
      log.warn('Failed to load offline queue from localStorage', error);
      this.queue = [];
    }
  }
  ...
  private saveQueue(): void {
    ...
    try {
      ...
      log.warn('Failed to save offline queue to localStorage', error);
    }
  }
    ...
    for (const request of this.queue) {
      try {
        ...
        if (request.retries >= (request.maxRetries ?? this.options.maxRetries)) {
          // 최대 재시도 횟수 초과 - 큐에서 제거
          log.warn(`Request ${request.id} exceeded max retries, removing from queue`, undefined, {
